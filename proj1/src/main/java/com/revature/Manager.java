...
public class Manager {
    ...
    public int insertSelf(Statement statement) {
    	if (!isInserted) {
    		String sql = "INSERT INTO managers (username, password, first_name, last_name) " +
                    "VALUES ('"+username+"', '"+password+"', '"+firstName+"', '"+lastName+"')";
        	try {
        		// add request into database
    			statement.executeUpdate(sql);
    			// find what id was given to this new entry
    			sql = "SELECT MAX(id) FROM managers";
    		...
    		System.err.println("Manager already inserted");
    		return -1;
    	}
    }
    public boolean deleteSelf(Statement statement) {
    	if (isInserted) {
    		String sql = "DELETE FROM managers WHERE id="+this.id;
        	try {
    		...
    		System.err.println("Manager not yet in table");
    		return false;
    	}
    }
    public int getId() {
